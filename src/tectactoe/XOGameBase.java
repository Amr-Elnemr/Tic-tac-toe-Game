...
//                        btn.setStyle("-fx-background-color:#cce6ff ");
    ...
    public void Winner(Boolean state, String name){
        if(state){
            ...
            ButtonType menu=new ButtonType("Main Menu",ButtonBar.ButtonData.OK_DONE);
            ButtonType cancel=new ButtonType("Cancel", ButtonBar.ButtonData.CANCEL_CLOSE);
            winalert.getButtonTypes().setAll(save,menu,cancel);
            ...
            Optional<ButtonType> result = winalert.showAndWait();  
                    ...
                    if (result.get() == save)
                    {
                         ...
                         String Result;
                           //check if winner is player1 set result =w  else l
                        ...
                        db.saveGame(multiPlayer.moves, player1, player2, Result);
                        ...
                        Optional<ButtonType> savedialog = alert.showAndWait();  
                        if(savedialog.get()==ok)
                        {
                          try
                           {
                             GameModesBase back = new GameModesBase(GameStage);
                             Scene scene = new Scene(back, 500, 400);
            ...
            ButtonType save=new ButtonType("Save Game",ButtonBar.ButtonData.OK_DONE);
            ButtonType menu=new ButtonType("Main Menu",ButtonBar.ButtonData.OK_DONE);
            winalert.getButtonTypes().setAll(save,menu);
            ...
            Optional<ButtonType> result = winalert.showAndWait();  
                    if (result.get() == save)
                    {
                        ...
                        Database db=new Database();
                        db.saveGame(multiPlayer.moves, player1, player2, "D");
                        Alert alert = new Alert(Alert.AlertType.INFORMATION);
                        alert.setTitle("Help");
                        alert.setHeaderText("How To Play");
                        alert.setContentText("Game Saved");
